...
Google mode only (plain or with the medical glossary): a packaged executable
has no agent and no API key, so the handoff engine is reachable from the
skill rather than from here.
"""
...
class App(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def _build_controls(self) -> None:
        ...
        # Swaps known clinical/rehabilitation terms for a curated Vietnamese
        # translation before the segment reaches Google, instead of leaving
        # medical vocabulary to Google's generic, context-free word choice.
        # See pdf2zh/medical_glossary.py for the term list.
        self.medical_mode = ctk.CTkCheckBox(
            controls, text="Ngữ cảnh y khoa (thuật ngữ y khoa/phục hồi chức năng)",
            checkbox_width=18, checkbox_height=18,
            font=ctk.CTkFont(self.ui_font, size=12),
        )
