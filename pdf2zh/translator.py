...
from pdf2zh.medical_glossary import protect_medical_terms, restore_medical_terms
...
class GoogleMedicalTranslator(GoogleTranslator):
    """Google Translate with a curated medical/rehabilitation glossary.

    Same request as ``GoogleTranslator``, except known clinical terms and
    abbreviations (``pdf2zh/medical_glossary.py``) are swapped for
    placeholder tags before the request and restored to their curated
    Vietnamese translation after, so Google's general-purpose word choice
    never gets applied to them. Every other word in the segment is still
    translated by Google exactly as before.
    """
    ...
    def do_translate(self, text: str) -> str:
        ...
        translated = super().do_translate(protected)
        return restore_medical_terms(translated, mapping)
